...
- 표시 이름은 **서버가 만든다.** 클라이언트가 보낸 `handle`, `id`, `createdAt`은 전부 무시된다.
  이름은 IP 해시(일별 회전)에서 파생한 `느긋한 수달` 형태(형용사+명사)이며 날마다 바뀐다.
...
interface ChatMessage {
  ...
  handle: string;     // "느긋한 수달" — 서버 생성, 일별 회전
...
IP 해시당 동시 소켓 수 제한(`CHAT_MAX_SOCKETS_PER_IP`, 기본 25)뿐이다.
...
| `CHAT_MAX_SOCKETS_PER_IP` | 25 | 같은 IP 해시의 동시 소켓 상한. 초과 시 429 |
...
Workers Free 플랜(SQLite DO) 실측 한도와 대조한 결과다.

| 자원 | 한도 | 이 방의 소비 | 결론 |
| --- | --- | --- | --- |
| Rows written | 100,000/일 | 메시지당 **3행**(본문 + 커서 + 500 초과 시 삭제) | **가장 먼저 막히는 자원.** 하루 약 33,000건 |
| Requests | 100,000/일 | 대시보드 폴링이 대부분(탭 1개 시간당 300) | 약 333 탭·시간 |
| Rows read | 5,000,000/일 | 입장당 백로그 50행 | 하루 10만 회 입장까지 여유 |
| Storage | 5 GB | 500개 롤링 | 무관 |

- 메시지 33,000건/일은 정규장 6.5시간에 몰아 쓰면 **분당 85건**이다. 1인 상한이 분당 15건이므로
  **동시에 활발히 말하는 사람 약 6명분**이 한계다. 그 이상이면 쓰기 한도가 먼저 닿는다.
- **keepalive는 과금되지 않는다**(`setWebSocketAutoResponse` 처리). 이는 설계 이득이며,
  일반 메시지 핸들러로 옮기면 접속자 수 × 45초 간격만큼 요청이 발생한다. 옮기지 말 것.
- 브로드캐스트는 `JSON.stringify`를 **1회만** 하고 소켓 수만큼 `send`한다. 송신은 무료이고
  CPU도 저렴해서 수천 소켓까지 여유가 있다. 이 구조를 소켓별 직렬화로 바꾸지 말 것.
- 다만 **입장·퇴장마다 presence를 전원에게 브로드캐스트**한다. N명이 동시에 재접속하면
  N×N 송신이 된다(배포 직후가 그런 상황이다). 수백 명 규모에서 문제가 되면
  presence 브로드캐스트를 디바운스해야 한다.

### 공유 IP의 구조적 한계

신원과 레이트리밋이 모두 **IP 해시 단위**다. 사무실·카페·학교, 그리고 CGNAT 뒤의
이동통신망은 하나의 공인 IP로 보인다. 따라서 같은 망의 사람들은:

1. **동시 소켓 상한을 공유한다.** 그래서 상한을 3 → 25로 올렸다.
2. **분당 15건 전송 예산을 공유한다.** 20명이 같은 망이면 1인당 분당 0.75건이다.
3. **표시 이름이 같다.** 같은 해시 → 같은 형용사+명사 조합.

2번과 3번은 계정 없이 익명을 유지하는 대가이며, 해소하려면 연결 단위 신원(계정 또는
서버 발급 세션)이 필요하다. 소유자 결정 사항이므로 코드로 임의 변경하지 않았다.
