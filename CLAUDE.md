...
* 야후 조회도 3회까지 재시도한다(`withRetries`). 한 번의 네트워크 실패로 그날 앵커가
  통째로 비는 것을 막는다.
* **GitHub cron은 보장이 아니다**(2026-08-27 실측). 이날 이 저장소의 예약 실행이
  전부 누락돼 밤 10시까지 카드가 전일 종가 기준이었다. baseline은 19:40·22:40 KST,
  경제 캘린더는 21:10 KST **캐치업 슬롯**을 둔다 — 스크립트가 멱등이라 정상일에는
  20초 no-op(커밋·배포 없음)이고, 누락일에만 실제로 일한다. 캐치업을 지우지 말 것.
