...
* **모바일 카드는 기준가까지 보여준다**(2026-08-12 소유자 결정). 계산에 쓰이는 세 줄
  — 국내 기준가·현재 해외 선물가·기준가 — 은 모두 남기고, 매수/매도 호가와 호가
  스프레드만 데스크톱 전용이다. 기준가를 숨기면 등락률의 분모가 화면에서 사라진다.
  `StockEstimateCard.test.tsx`의 "metric rows on mobile"이 이 구성을 고정한다.
...
### 28.6 홈 화면 추가 버튼 (2026-08-13)

모바일에서만 오른쪽 아래에 떠 있는 "홈 화면에 추가" 버튼(`InstallButton`).
