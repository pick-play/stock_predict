...
* **공유 이미지는 카드와 같은 모양을 따른다**: 배지 대신 가격 아래 등락 한 줄,
  3행 패널 대신 종가 한 줄. **스파크라인 색은 시리즈의 첫 점 대비 마지막 점**으로
  정한다 — 카드 등락 방향(예상가 vs 기준가)으로 칠하면 최근 몇 시간이 하루와 반대로
  움직일 때 방금 본 화면과 다른 색의 그림이 저장된다.
* 차트 기본 범위는 **6시간**이다. 24시간은 사이트가 무시하라고 안내하는 국내 정규장
  구간까지 포함한다.
