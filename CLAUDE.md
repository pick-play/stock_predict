...
* `public/og-image.jpg`는 2026-08-31 새 로고 기반으로 교체되어 "코스피 NOW" 표기를 따른다.
  원본은 `design/share-image-source.png`(1734×907)로, 처음부터 OG 비율(1.91:1)로
  그려져 배포본은 1200×630 축소뿐이다. 이 이미지에는 도메인 텍스트가 없다 —
  소유자가 그렇게 그린 것이고, OG 미리보기는 이미지 밑에 URL을 항상 함께 보여준다.
  28.4의 "kospinow.com을 지운다 금지"는 카드 공유 이미지(SharePreviewModal) 얘기이며
  그쪽은 그대로다.
...
* **야간 등락 ±8% 상한은 제거했다**(2026-08-31 소유자 결정으로 2026-08-22 결정을
  뒤집음). ±8%는 국내 야간거래(코스피 야간 세션)의 상하한폭인데, 이 사이트가
  추종하는 것은 상하한가가 없는 **해외 선물 연계 계약**이지 코스피200 야선이
  아니다 — 애초에 적용 대상이 아닌 시장 규칙이었고, 잘린 숫자는 해외 시장이
  실제로 얼마나 움직였는지를 감췄다. `NIGHT_SESSION_LIMIT_RATE`·`limited` 필드는
  코드에서 삭제됐다. 되살리려면 그때는 표시(`limited` 캡션)까지 함께 구현할 것 —
  상한이 있던 기간 내내 잘렸다는 표시가 화면에 없어서, 잘린 -8.00%가 계산의
  원래 답인 것처럼 보였다(감사에서 발견). 클램프 없이도 ±30% 이상치 가드
  (`MAX_CHANGE_RATE`)는 데이터 품질 규칙으로 별개로 남아 있다.
...
### 28.9 정적 정보 페이지·연락처 (2026-08-31)

* 애드센스 1차 심사 탈락을 계기로 **정적 정보 페이지 5개**를 추가했다:
  `/about/`(소개·연락처), `/guide/`(계산 방식·호가단위), `/faq/`,
  `/privacy/`(개인정보처리방침), `/terms/`(이용약관). 탈락의 실측 원인은
  ① SPA라 크롤러가 보는 본문이 72단어뿐, ② 색인 가능한 URL이 1개,
  ③ 개인정보처리방침 부재(회원·세션·GA를 운영하면서 법적으로도 필요한 상태)였다.
* 다섯 페이지는 **404.html과 같은 독립 정적 파일**이다(`public/<이름>/index.html`).
  번들·애널리틱스·광고 없음, CSS 인라인, 테마 부트 스크립트는 index.html과 동일.
  **JS 없이 전문이 읽히는 것이 존재 이유**이므로 SPA 라우트로 옮기지 말 것.
  실제 경로라 404의 리다이렉트 목록(/board·/chat)과는 무관하다.
* 공개 연락처 이메일은 **kospinow@gmail.com**이다. about·faq·privacy·terms와
  각 페이지 푸터에 실려 있다.
* 대시보드 푸터(`Disclaimer.tsx`)가 다섯 페이지로 실제 `<a>` 링크를 건다 —
  해시 라우트가 아니므로 SPA 네비게이션을 쓰지 않는 것이 맞다. sitemap.xml에도
  다섯 URL이 실경로로 들어 있다("URL 1개" 주석은 앱 자체에 대한 이야기로 갱신됨).
* privacy의 내용은 실제 구현과 일치해야 한다: 비밀번호 이중 해시, 원본 IP 미보관
  (일별 회전 해시), 채팅 500개 롤링, 세션 30일, 로그인 시도 기록 24시간,
  이메일·실명 미수집. **저장 방식이 바뀌면 이 페이지도 같이 고칠 것** — 방침이
  코드와 다르면 없는 것보다 나쁘다.
* 사용자 노출 문구 규칙(§28: "바이낸스" 대신 "해외 선물가격" 계열)은 이 페이지들에도
  적용된다. privacy의 데이터 출처 항목도 "외부 시세 제공자"로만 쓴다.
* **정보 페이지 어디에도 GitHub을 언급하지 않는다**(2026-08-31 소유자 결정).
  privacy의 호스팅 항목은 "웹 호스팅 사업자"로만 쓴다 — 소유자는 저장소가
  특정되는 것을 원치 않는다. 단, 저장소 자체가 public이라는 사실은 별개 문제로
  소유자에게 보고됨(무료 Pages의 전제 조건).
* 콘텐츠 확장(같은 날, 애드센스 재신청 대비): **/stocks/**(목차 + 7종목 각각
  samsung / sk-hynix / hyundai / samsung-em / lg-electronics / hanmi / naver)와
  **/glossary/**(목차 + night-futures / futures-vs-spot / single-price-auction /
  tick-size / close-open-price / kospi-kospi200). 전부 같은 독립 정적 패턴이고
  sitemap에 실경로로 등록됨. 종목 페이지에는 구체 재무 수치(매출액·시총 금액)를
  쓰지 않는다 — 낡으면 틀린 주장이 된다. 정성적·안정적 사실만.
* `public/ads.txt`에 애드센스 게시자 ID가 있다(승인 후 수익 경고 방지용 표준 파일).
* 사이트 전역 푸터 내비게이션은 8개 링크다: 홈 · 서비스 소개 · 이용 가이드 ·
  자주 묻는 질문 · 종목 소개 · 용어 설명 · 개인정보처리방침 · 이용약관.
  정적 페이지와 `Disclaimer.tsx`(대시보드 푸터)가 같은 목록을 쓴다.
