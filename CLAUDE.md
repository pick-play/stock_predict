...
* 다크 모드 기본을 유지하되, `prefers-color-scheme: light` 및 `data-theme` 속성 기반 라이트 모드를 지원한다.
...
### 28.1 브랜드·도메인 (2026-08-09)

* 서비스명은 **KOSPI NOW**(한글 병기 "코스피 나우")다. 기존 "야간 반도체 예상가"는 사용하지 않는다.
  20장의 페이지 제목 예시는 `KOSPI NOW | 삼성전자·SK하이닉스 야간 참고가격`으로 대체한다.
* 헤더 로고 아래 보조 문구는 "코스피 나우"다.
...
* 로그인이 없으므로 남용 방어선은 다음 셋뿐이며, **세 가지 모두 서버에서 강제한다.**
  1. 입장 시 Turnstile 1회(30분 수명의 입장 티켓으로 교환, IP 해시에 서명 결합)
  2. IP 해시 단위 전송 제한(2초 간격, 분당 15건)
  3. `moderatePost()` 검열 + 길이·공백 검사
  이 중 하나라도 제거하면 방은 사실상 무방비다.
...
* **모바일 헤더는 다크모드·토론방·실시간 채팅을 오버플로 메뉴로 감춘다**(`HeaderMenu`).
  ...
  데스크톱은 인라인을 유지한다.
* 메인의 "토론"·"최근 채팅" 스트립은 **삼성전자·SK하이닉스 카드 아래에 좌우로** 배치한다.
  카드 위가 아니다 — 모바일에서 가격보다 먼저 나오면 정보량이 과해진다.
  두 스트립은 **카드 전체가 해당 페이지로 이동하는 버튼**이다.
  인기글 스트립 안의 공감 버튼은 `stopPropagation`으로 이동을 막는다.
  이 방어를 지우면 공감 한 번에 화면이 토론방으로 튄다.
