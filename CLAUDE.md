...
* **표시되는 접속자 수는 사이트 전체 접속자다**(2026-08-22 소유자 결정으로 "열린
  ...
  설명한다.
  방(`ChatRoom`)이 `sitePresence: Map<ipHash, lastSeen>`를 **메모리에만** 들고,
  열린 소켓의 IP 해시와 **합집합**으로 센다(방에 있는 사람도 사이트에 있는 사람이므로
  두 번 세면 안 된다). 저장소에 쓰지 않는 이유는 sendHistory와 같지만 더 강하다 —
  방문자당 분당 1행이면 무료 플랜에서 가장 먼저 막히는 rows written을 정면으로 쓴다.
...
* **숫자가 바뀌면 방이 소켓으로 알린다.** 이전에는 presence 프레임이 소켓 열림·닫힘
  ...
  조용한 사이트에서는 0건이다.
* 소켓이 없는 화면(대시보드 스트립)은 `useSiteCount()`가 `GET /api/chat/count`를
  **5초마다, 보이는 탭에서만** 읽는다(소유자 결정). 메시지 없이 숫자만 담아 18바이트다.
  **방 읽기는 여기에 비례해 늘지 않는다** — 워커가 전원이 공유하는 캐시
  (`CHAT_COUNT_TTL_MS`, 5초)로 답하므로 몇 명이 보든 캐시 주기당 1회다.
  캐시 수명을 폴링 주기보다 길게 잡으면 같은 숫자를 두 번 주게 되어 빠른 폴링이
  통째로 낭비가 된다 — 두 값은 함께 움직여야 한다.
